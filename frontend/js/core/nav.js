...
(function (C) {
  ...
  function iconOcultar() {
    return `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7M21 12H8M4 5v14"/></svg>`;
  }

  const CHAVE_MENU = 'campanha.menu-lateral';

  function menuLateralOculto() {
    try {
      return localStorage.getItem(CHAVE_MENU) === 'oculto';
    } catch {
      return false;
    }
  }

  function aplicarMenuLateral() {
    const oculto = menuLateralOculto();
    const app = $('tela-app');
    if (app) app.classList.toggle('sidebar-oculto', oculto);
  }

  function definirMenuLateral(oculto) {
    try {
      localStorage.setItem(CHAVE_MENU, oculto ? 'oculto' : 'visivel');
    } catch (_) { /* ignore */ }
    aplicarMenuLateral();
  }
  ...
  C.nav = {
    montar() {
      ...
      side.innerHTML = `
        ...
        ` : ''}
        <button type="button" data-ocultar-menu
          class="mt-6 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-white/70 transition hover:bg-white/10 hover:text-white">
          ${iconOcultar()}
    ...
    bind() {
      ...
      if (side) {
        side.addEventListener('click', (ev) => {
          if (ev.target.closest('[data-ocultar-menu]')) {
            definirMenuLateral(true);
            return;
          }
          const btn = ev.target.closest('[data-nav]');
      ...
      const btnMostrar = $('btn-mostrar-menu');
      if (btnMostrar) {
        btnMostrar.addEventListener('click', () => definirMenuLateral(false));
      }
