-- Endereço da entrega: usar o do colaborador ou cadastrar um novo.
...
SET
  endereco = COALESCE(e.endereco, c.endereco),
  numero = COALESCE(e.numero, c.numero),
  complemento = COALESCE(e.complemento, c.complemento),
  bairro = COALESCE(e.bairro, c.bairro),
  cidade = COALESCE(e.cidade, c.cidade),
  estado = COALESCE(e.estado, c.estado)
FROM public.colaboradores c
WHERE c.id = e.colaborador_id
  AND e.endereco IS NULL
  AND e.numero IS NULL
  AND e.complemento IS NULL
  AND e.bairro IS NULL
